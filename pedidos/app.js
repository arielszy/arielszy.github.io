...
async function syncWithServer() {
    ...
    try {
        ...
        for (const item of unsyncedItems) {
            try {
                const response = await fetch(`${serverUrl}/${item.store}`, {
        ...
        lastSyncTime = new Date().toLocaleString('es-ES');
...
function updateSyncIndicator() {
    ...
    if (navigator.onLine) {
        ...
        status.innerHTML = '<span class="sync-indicator online"></span>En línea';
    } else {
...
function updateSyncStatus() {
    const serverUrl = localStorage.getItem('serverUrl');
    document.getElementById('syncStatus2').innerHTML = `
        <strong>Servidor:</strong> ${serverUrl || 'No configurado'}<br>
        <strong>Estado:</strong> ${navigator.onLine ? '✅ En línea' : '❌ Sin conexión'}
    `;
    document.getElementById('lastSync').innerHTML = `<strong>Última sincronización:</strong> ${lastSyncTime || 'Nunca'}`;
}
...
function saveServerConfig() {
    ...
    if (!serverUrl) {
        alert('❌ Ingrese URL del servidor');
        return;
    }

    localStorage.setItem('serverUrl', serverUrl);
    localStorage.setItem('apiKey', apiKey);
    alert('✅ Configuración del servidor guardada');
    updateSyncStatus();
}
...
// ============ VARIABLES ============
let editingClientId = null, editingOrderId = null;
...
function updateAmountRequirement() {
    ...
    if (status === 'facturado' || status === 'entregado') {
        ...
        amountHint.textContent = 'Campo obligatorio para este estado';
    } else {
...
async function deleteClient(id) {
    ...
    try {
        ...
        await addToSyncQueue('delete', 'clients', { id });
...
async function saveOrder() {
    ...
    if ((status === 'facturado' || status === 'entregado') && amount <= 0) {
        showAlert('alertPedidos', 'El monto es obligatorio para pedidos facturados o entregados', 'error');
        return;
    }
...
async function deleteOrder(id) {
    ...
    try {
        ...
        await addToSyncQueue('delete', 'orders', { id });
        await renderOrders();
...
async function addPayment() {
    ...
    if (!clientId || amount <= 0) { showAlert('alertPagos', 'Complete campos', 'error'); return; }
...
async function deletePayment(id) {
    ...
    try {
        ...
        await addToSyncQueue('delete', 'payments', { id });
        await renderPayments();
        await renderAccountsStatus();
